...
Behind the maps and charts on [Peskas Coasts](https://coasts.peskas.org) is an automated system. Every day, a schedule is triggered via GitHub Actions that runs the **Peskas Coasts Pipeline**. 
...
* **`merge_survey_trips()`**: Here, we collect the landing surveys that each country pipeline has already checked and matched to GPS trips. If a surveyor recorded a catch at the dock, it is now linked to the GPS device that was on that specific boat.
...
### Summary
Every day, the pipeline processes raw pings from ocean trackers and landing surveys into an updated spatial dataset.