...
.download_tracks <- function(
  ...
  if (nrow(file_list) == 0) {
    stop("No files to download for {label}.")
  }
...
build_catch_wide <- function(matched) {
  ...
  catch_wide <- matched |>
    ...
    dplyr::summarise(
      catch_kg = sum(catch_kg, na.rm = TRUE),
      .groups = "drop"
    ) |>
    tidyr::pivot_wider(
      id_cols = pds_trip,
...
run_weighted_cpue <- function(trips, top_n, min_trips) {
  ...
  catch_per_trip <- trips |>
    dplyr::group_by(trip) |>
    dplyr::summarise(
      dplyr::across(dplyr::all_of(top_species), \(x) {
...
model_cpue <- function(
  ...
  tracks_m <- download_matched_tracks(
    conf,
    matched,
  ...
  map_effort <- plot_effort_map(
    effort_all,
    title = glue(
      "{stringr::str_to_title(country)} - Fishing Effort by Year (all trips)"
    )
  )
